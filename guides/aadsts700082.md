...
expiredorrevokedgrantinactivetoken - the refresh token has expired due to
inactivity. the token was issued on {issuedate} and was inactive for {time}.
expected part of the token lifecycle - the user went an extended period of time
without using the application, so the token was expired when the app attempted
to refresh it..
